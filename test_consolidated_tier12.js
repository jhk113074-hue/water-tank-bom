...
function startLocalServer() {
  return new Promise((resolve) => {
    ...
    server.listen(8128, () => resolve(server));
  });
}
...
async function run() {
  ...
  console.log('Navigating to http://localhost:8128/#pallet-packing ...');
  await page.goto('http://localhost:8128/#pallet-packing', { waitUntil: 'networkidle2' });
  ...
  await page.screenshot({ path: path.join(artifactDir, 'test_consolidated_tier12_chrome.png') });
  console.log('Captured test_consolidated_tier12_chrome.png');

  const tierListText = await page.evaluate(() => {
    const tiers = Array.from(document.querySelectorAll('#palletsDashboard .tier-item, #palletsDashboard [style*="border-radius"]'));
    return tiers.map(t => t.textContent.trim()).filter(t => t.length > 0);
  });
  console.log('Tiers in Dashboard:', tierListText.slice(0, 10));
