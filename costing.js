...
(function (global) {
  ...
  const SEED_PANEL_ROWS = [
    ...
    return Object.assign({
      manualNonInsulatedCost: null,
      insulation25: { skinKg: 0, mdiKg: 0, polyolKg: 0, processingCost: 0, manualCost: null },
      insulation40: { skinKg: 0, mdiKg: 0, polyolKg: 0, processingCost: 0, manualCost: null }
    }, r);
  });
  ...
  const DEFAULT_STATE = {
    ...
    panelRows: SEED_PANEL_ROWS.map(function (r) { return deepClone(r); })
  };

  let state = null;
  let activeSubTab = "labor";
  let activePanelView = "base"; // "base" | "ins25" | "ins40" -- which columns 패널 원가표 shows
  ...
  // Computes one thickness tier's (25mm/40mm) insulation cost on top of the
  // shared bare-panel cost. Each tier keeps its own Skin/MDI/POLYOL kg usage
  // and processing cost, since a thicker panel uses more insulation material
  // -- but both tiers share the same $/kg material rates (Material Cost tab).
  function computeInsulationTier(ins, calcNonInsulatedCost, rates) {
    ins = ins || {};
    const insulationRawCost = (ins.skinKg || 0) * rates.insulSkinPerKg + (ins.mdiKg || 0) * rates.mdiPerKg + (ins.polyolKg || 0) * rates.polyolPerKg;
    const insulationCostSum = insulationRawCost + (ins.processingCost || 0);
    const calcInsulatedCost = calcNonInsulatedCost + insulationCostSum;
    const hasManual = ins.manualCost !== null && ins.manualCost !== undefined && ins.manualCost !== "";
    return {
      insulationRawCost: insulationRawCost,
      insulationCostSum: insulationCostSum,
      calcInsulatedCost: calcInsulatedCost,
      insulatedCost: hasManual ? Number(ins.manualCost) : calcInsulatedCost,
      isInsulatedManual: hasManual
    };
  }
  ...
  function computeRow(row, rates) {
    ...
    return {
      ...
      // Final value used everywhere (table display + DB 반영): manual
      ...
      insul25: computeInsulationTier(row.insulation25, calcNonInsulatedCost, rates),
      insul40: computeInsulationTier(row.insulation40, calcNonInsulatedCost, rates)
    };
  }
  ...
  function normalizeState() {
    ...
    state.panelRows = state.panelRows.map(function (r) {
      const row = Object.assign({ manualNonInsulatedCost: null }, r);
      // Migrate a pre-existing saved project's old single-tier insulation
      // fields (insulSkinKg/mdiKg/polyolKg/insulationProcessingCost/
      // manualInsulatedCost) into the 25mm tier, so older saves aren't
      // silently zeroed out; 40mm starts blank either way (no prior data).
      const legacy25 = {
        skinKg: row.insulSkinKg || 0, mdiKg: row.mdiKg || 0, polyolKg: row.polyolKg || 0,
        processingCost: row.insulationProcessingCost || 0,
        manualCost: (row.manualInsulatedCost === undefined) ? null : row.manualInsulatedCost
      };
      row.insulation25 = Object.assign({ skinKg: 0, mdiKg: 0, polyolKg: 0, processingCost: 0, manualCost: null }, row.insulSkinKg !== undefined ? legacy25 : row.insulation25);
      row.insulation40 = Object.assign({ skinKg: 0, mdiKg: 0, polyolKg: 0, processingCost: 0, manualCost: null }, row.insulation40);
      delete row.insulSkinKg; delete row.mdiKg; delete row.polyolKg; delete row.insulationProcessingCost; delete row.manualInsulatedCost;
      return row;
    });
    const knownCodes = {};
    state.panelRows.forEach(function (r) { knownCodes[(r.code || "").toUpperCase()] = true; });
    SEED_PANEL_ROWS.forEach(function (seed) {
      if (!knownCodes[seed.code]) state.panelRows.push(deepClone(seed));
    });
  }
  ...
  window.switchCostingPanelView = function (view) {
  ...
  window.updateCostingPanelRowField = function (idx, field, value) {
    ...
    if (field === "code" || field === "name" || field === "tankHeight") {
      row[field] = value;
    } else if (field === "manualNonInsulatedCost") {
  ...
  // field: 'skinKg' | 'mdiKg' | 'polyolKg' | 'processingCost' | 'manualCost'
  window.updateCostingPanelInsulField = function (idx, thickness, field, value) {
    const row = state.panelRows[idx];
    if (!row) return;
    const key = thickness === "40" ? "insulation40" : "insulation25";
    if (!row[key]) row[key] = { skinKg: 0, mdiKg: 0, polyolKg: 0, processingCost: 0, manualCost: null };
    if (field === "manualCost") {
      row[key][field] = (value === "" || value === null || value === undefined) ? null : parseFloat(value);
    } else {
      row[key][field] = parseFloat(value) || 0;
    }
    persist(global.db);
    render();
  };
  ...
  window.addCostingPanelRow = function () {
    const row = deepClone(NEW_ROW_TEMPLATE);
  ...
  function applyRowsToDb(rows) {
    ...
    rows.forEach(function (row) {
      ...
      global.partsDb.forEach(function (p, idx) {
        if (p && p.category === "PANEL" && p.partNo && p.partNo.toUpperCase().indexOf(upperCode) === 0) {
          ...
          global.updateDbField(idx, "priceInsulated25", c.insul25.insulatedCost.toFixed(2));
          global.updateDbField(idx, "priceInsulated40", c.insul40.insulatedCost.toFixed(2));
          updatedCount++;
          matched = true;
        }
      });
    ...
    alert(codeCount + "개 패널 코드, 총 " + updatedCount + "개 DB 항목에 단판/보온(25mm/40mm) 단가가 반영되었습니다.");
    render();
  }
  ...
  function panelViewBtn(view, label) {
    const active = activePanelView === view;
    return '<button type="button" class="bom-sub-btn' + (active ? ' active' : '') + '" onclick="switchCostingPanelView(\'' + view + '\')" style="padding: 4px 10px; font-size: 11.5px; font-weight: 700; border: none; border-radius: 6px; cursor: pointer; background: ' + (active ? '#ffffff' : 'transparent') + '; color: ' + (active ? '#0284c7' : '#64748b') + '; box-shadow: ' + (active ? '0 1px 2px rgba(0,0,0,0.1)' : 'none') + '; white-space: nowrap;">' + label + '</button>';
  }

  function renderPanelRowBase(row, idx, c, matches) {
    return '<tr>' +
      ...
      '<td align="center" style="display:flex; gap:6px; justify-content:center;">' +
        '<i class="fa-solid fa-arrow-right-to-bracket action-icon" title="이 행만 DB 반영 (단판+보온25mm+보온40mm)" onclick="applyCostingRowToDb(' + idx + ')" style="color:#0284c7; cursor:pointer; padding:6px;"></i>' +
  ...
  function renderPanelRowInsul(row, idx, c, matches, thickness) {
    const key = thickness === "40" ? "insul40" : "insul25";
    const insField = thickness === "40" ? "insulation40" : "insulation25";
    const ins = row[insField] || {};
    const cc = c[key];
    return '<tr>' +
      '<td style="font-weight:700;">' + (row.code || "") + '</td>' +
      '<td>' + (row.name || "") + '</td>' +
      '<td>' + fmtMoney(c.nonInsulatedCost) + '</td>' +
      '<td>' + numInput(ins.skinKg, "updateCostingPanelInsulField(" + idx + ",'" + thickness + "','skinKg',this.value)") + '</td>' +
      '<td>' + numInput(ins.mdiKg, "updateCostingPanelInsulField(" + idx + ",'" + thickness + "','mdiKg',this.value)") + '</td>' +
      '<td>' + numInput(ins.polyolKg, "updateCostingPanelInsulField(" + idx + ",'" + thickness + "','polyolKg',this.value)") + '</td>' +
      '<td>' + numInput(ins.processingCost, "updateCostingPanelInsulField(" + idx + ",'" + thickness + "','processingCost',this.value)") + '</td>' +
      '<td>' + fmtMoney(cc.insulationCostSum) + '</td>' +
      '<td>' + overrideInput(ins.manualCost, "updateCostingPanelInsulField(" + idx + ",'" + thickness + "','manualCost',this.value)") + '</td>' +
      '<td><b>' + fmtMoney(cc.insulatedCost) + '</b>' + (cc.isInsulatedManual ? ' <span class="badge" title="수동 조정값 사용 중">수동</span>' : '') + '</td>' +
      '<td align="center"><span class="badge">매칭 ' + matches + '건</span></td>' +
      '<td align="center" style="display:flex; gap:6px; justify-content:center;">' +
        '<i class="fa-solid fa-arrow-right-to-bracket action-icon" title="이 행만 DB 반영 (단판+보온25mm+보온40mm)" onclick="applyCostingRowToDb(' + idx + ')" style="color:#0284c7; cursor:pointer; padding:6px;"></i>' +
        '<i class="fa-solid fa-trash-can action-icon" title="행 삭제" onclick="deleteCostingPanelRow(' + idx + ')" style="color:var(--neon-rose); cursor:pointer; padding:6px;"></i>' +
  ...
  function renderPanelTab() {
    const rates = currentRates();
    const isBase = activePanelView === "base";
    const thickness = activePanelView === "ins40" ? "40" : "25";
    let rows = "";
    state.panelRows.forEach(function (row, idx) {
      const c = computeRow(row, rates);
      const matches = matchingDbCount(row.code);
      rows += isBase ? renderPanelRowBase(row, idx, c, matches) : renderPanelRowInsul(row, idx, c, matches, thickness);
    });

    const viewNav = '<div style="display:flex; gap:6px; background:#eef2f7; padding:3px; border-radius:7px; width:fit-content; margin-bottom:12px; flex-wrap:wrap;">' +
      panelViewBtn("base", "기본 (단판)") + panelViewBtn("ins25", "보온 25mm") + panelViewBtn("ins40", "보온 40mm") +
      '</div>';
    ...
    const thead = isBase ?
      ('<th>코드</th><th>품명</th><th>탱크높이</th><th>WT(kg)</th><th>G/C(g)</th>' +
       ...
       '<th>DB매칭</th><th></th>') :
      ('<th>코드</th><th>품명</th><th>단판원가</th>' +
       '<th>보온Skin(kg)</th><th>MDI(kg)</th><th>POLYOL(kg)</th><th>보온가공비</th><th>보온원가합계</th>' +
       '<th>보온원가(수동)</th><th>보온원가(' + thickness + 'mm)</th><th>DB매칭</th><th></th>');
    ...
    return '' +
      ...
      '<i class="fa-solid fa-triangle-exclamation"></i> "단판원가(수동)"/"보온원가(수동)" 칸에 값을 입력하면 자동 계산 대신 그 값이 최종 원가로 사용됩니다(주황 테두리). 비워두면 다시 자동 계산됩니다. 25mm/40mm 탭은 서로 다른 보온재 사용량을 각각 따로 관리합니다.' +
      '</div>' +
      viewNav +
      '<div style="display:flex; gap:8px; margin-bottom:10px; flex-wrap:wrap;">' +
      '<button class="btn btn-primary" onclick="applyAllCostingToDb()"><i class="fa-solid fa-database"></i> 전체 DB 반영 (단판+보온25mm+보온40mm)</button>' +
      '<button class="btn btn-outline" onclick="addCostingPanelRow()"><i class="fa-solid fa-plus"></i> 행 추가</button>' +
      '</div>' +
      '<div class="table-wrapper"><table class="bom-table"><thead><tr>' + thead + '</tr></thead><tbody>' + rows + '</tbody></table></div>';
  }
