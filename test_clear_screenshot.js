...
function startLocalServer() {
  return new Promise((resolve) => {
    ...
    server.listen(8127, () => resolve(server));
  });
}
...
async function run() {
  ...
  console.log('Navigating to http://localhost:8127/#bom-output ...');
  await page.goto('http://localhost:8127/#bom-output', { waitUntil: 'networkidle2' });
  ...
  console.log('Clicking Clear All button with confirm accepted...');
  await page.evaluate(() => {
    window.confirm = () => true;
  ...
  await page.screenshot({ path: path.join(artifactDir, 'test_clear_all_verified.png') });
  console.log('Captured test_clear_all_verified.png');
