...
| [4] 품번별 교집합 — **Internal** (앱 기본값) | ✅ **조정 대상 19종 전부 Δ=0, 편차 0.0%** |
| [4] 품번별 교집합 — **External** | ✅ **조정 대상 6종 전부 Δ=0, 편차 0.0%** |

4mH 까지는 **양쪽 보강 방식 모두 공통 품번이 전부 정확히 일치**합니다. `WCA-*`(코너 프레임) 3종만 차이가 남는데, 이는 조정 대상이 아니라 **의도된 일반화**입니다(아래 ②) — 검증 스크립트도 "(의도된 차이)" 로 표시하고 통계에서 제외합니다.
...
### 남은 것 — 아직 이 스펙이 다루지 않는 부재

4mH 까지 수량은 맞췄지만, 기존 엔진이 내는 부재 중 이 스펙에 아직 없는 것들이 있습니다. 오차가 아니라 **커버리지 공백**입니다.

| 모드 | 미포함 품번 |
|---|---|
| Internal | `WBR-1740Z`(Side Bottom Bracket) · `WFB-0950ZL`(ZL 계열 앵글) · `WCP-1460SA4` |
| External | `WBR-75120Z`(Stopper Bracket) · `WBR-12527Z`(Roof Tie-rod Bracket) · `WBR-1750Z`(Lower fixture) · `WFR-*`(프레임류) · `WCB-7070Z` 등 29종 |

External 쪽 공백이 큰 것은 기존 external 보강 규칙이 프레임·스토퍼·하부고정 계열을 많이 포함하는데, 도면 1~11장에는 그 부재들이 별도 시트로 나오지 않기 때문입니다. 다음 차수에서 도면과 대조해 시트를 추가할지, 아니면 기존 규칙에 계속 맡길지 결정하면 됩니다.

### 다음 차수 (4.5mH / 5mH)

`--with-tall` 로 포함해 확인할 수 있습니다. 위 ①의 위상 전환과 ②의 코너 프레임 누락이 이 구간에 집중되어 있어, 4mH 까지 확정된 구조 위에서 별도로 진행합니다.
