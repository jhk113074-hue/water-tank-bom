...
| [2d] 코너 프레임 등가성 (총 코너 길이) | ✅ 통과 (70건) |
| [2b] 기존 검증 엔진과 정확 일치 | ✅ 통과 (140건) |
| [4] 품번별 교집합 — **Internal** (앱 기본값) | ✅ **22종 전부 Δ=0** / 편차 0.0% / 공백 0 |
| [4] 품번별 교집합 — **External** | ✅ **35종 전부 Δ=0** / 편차 0.0% / 공백 0 |

4mH 까지 **양쪽 보강 방식 모두, 공통 품번 전부가 정확히 일치하고 커버리지 공백이 0** 입니다. 즉 기존 엔진이 내는 보강 부재를 이 스펙이 하나도 빠뜨리지 않고, 수량도 전부 같습니다.

`WCA-*`(코너 프레임) 3종만 조각 구성이 다르지만 이는 **의도된 일반화**이고, 검증 [2d]가 **총 코너 길이 = 4 × 탱크 높이** 로 70건 전부 등가임을 실제로 계산해 확인합니다.
...
`--with-tall` 로 포함해 확인합니다. 이 구간에 두 가지가 집중되어 있습니다:

- **위상 전환** (위 ①) — 둘레 밴드 → 전면 그리드. S12 가 이미 이식했지만 S1/S3 도 같은 전환이 필요한지 확인해야 합니다.
- **코너 프레임 누락** (위 ②) — `--with-tall` 로 보면 기존 엔진이 코너 프레임을 0개 산출하는 케이스가 **정확히 10건**, 전부 **External 모드의 4.5/5mH** 입니다. Internal 모드는 4.5M(12개)/5M(8개)를 내므로, 누락은 External 쪽에 국한됩니다.
