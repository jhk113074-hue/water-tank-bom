...
| 영역 | BOM 산출 주체 |
|---|---|
| **보강재** (`group: "reinforcing"`) | **이 스펙** — `app.js` 가 `SteelAccessoriesEngine.partsOf(res, ['reinforcing'])` 로 산출 |
| 타이로드 · 실링테이프 · 부속자재 | 기존 엔진 (`tieRodInternalParts`, `sealingTapeDetail`, `airVent`, `roofSupporter` …) |

섹션마다 `group` 이 붙어 있어 필요한 서브시스템만 골라 쓸 수 있습니다. **통째로 쓰면 안 됩니다** — 타이로드·테이프·부속자재는 app.js 가 이미 별도 라인으로 넣고 있어 이중 계상됩니다.

스펙 계산이 실패하면 기존 `AccessoriesEngine.reinforcingParts` 로 자동 폴백합니다.

### 대체 결과 — 코너 프레임만 달라지고, 그마저 더 낫습니다

보강재 BOM 을 실제로 비교하면 **조각 구성이 바뀌는 것은 코너 프레임(`WCA-*`) 하나뿐**입니다. 그리고 그 변화는 개선입니다.

코너 높이를 1000/1500/2000mm 조각으로 채우는 방식을 **"최소 조각 수 → 동수면 최저 원가"** 로 바꿨습니다. 1500mm 이 미터당 가장 싸기 때문에(3.40 / 2.83 / 3.40 원/m) 동수일 때 1500 을 쓰는 쪽이 유리합니다.

| 높이 | 코너 1곳당 구성 |
|---|---|
| 1M / 1.5M / 2M | 1000×1 / 1500×1 / 2000×1 |
| 2.5M | 1500+1000 |
| 3M | 1500×2 |
| 3.5M | 2000+1500 |
| 4M | 2000×2 |
...
- **길이·조각 수·중량: 전 케이스 동일** (카탈로그 중량이 길이에 정비례)
- **원가: 변동 5건, 전부 절감(−6.80), 증가 0건** — 3M Internal 에서 `1000+2000` → `1500×2` 로 바뀌며 저렴해집니다
- **External 4.5/5mH: 기존이 0개였던 코너 프레임이 채워집니다** (원본 상수표의 누락 보완)

### 이 환경에서 확인하지 못한 것

BOM 전체 생성(`generateDefaultBOMFromConfig`)은 이 샌드박스에서 실행할 수 없습니다 — CDN 차단으로 `firebase is not defined` 가 나면서 app.js 초기화가 중단되고 `bomItems` 가 미초기화 상태가 됩니다. 탭 전환이 안 되는 것도 같은 원인이며 기존 탭들도 동일합니다. 그래서 **엔진 레벨에서 보강재 부재 목록·수량·중량·원가를 직접 대조**해 확인했습니다. 실제 환경에서 BOM 을 한 번 생성해 Reinforcing 라인을 확인해 주시면 좋겠습니다.

## 8. 아직 하지 않은 것

- **Rule Editor UI 연동 없음.** `SteelAccessoriesEngine.variables` 와 `profileOptions()` 가 연동용으로 준비되어 있습니다.
- **신규 등록 품번 3종의 중량·단가가 0** — `TR-14M2000SA2/SA4`, `WCP-17120Z`. 원가 산출 전 입력 필요.
- **도면 대조 확인 2건** (둘 다 도면엔 있고 워크북엔 없어, 워크북 누락 가능성):
  - 4.5M 이상 최하부 타이로드의 **14mm 직경 승급**
  - `S9.pw0455ZP` (격벽 라인 바닥측 0455ZP)
- **부속자재(S11) 도면 대조** — 앵커볼트·라이너·스키드 조이너는 대조할 상대가 없어 도면 문구와 눈으로 맞추는 것만 가능합니다.
