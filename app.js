...
function renderSidePanelConfig() {
  ...
  // Renders one compact editable box
    ...
    const variantsHtml = variants.map(v => {
      ...
      return `
        <div style="display:flex; align-items:center; gap:2px; margin-top:2px; min-width:0;" title="${tag}">
          <span style="font-size:8px; font-weight:700; color:#475569; flex:0 0 22px; text-align:right; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${tag}</span>
          ${makeSelectElement(v.idx, hGrade, vVal)}
        </div>`;
    }).join('');
    return `
      <div style="background:${palette.bg}; border:1px solid ${palette.border}; border-radius:4px; padding:3px 2px; box-sizing:border-box; width:100%; min-width:0; margin-bottom:3px;">
        <div style="font-size:8.5px; font-weight:800; color:${palette.text}; margin-bottom:2px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${boxLabel}">${boxLabel}</div>
  ...
  sideHeightGrades.forEach(hGrade => {
    ...
    roofHtmlMap[hGrade] = roleBox('roof_bottom.roof_full', ['roof_bottom.roof_half', 'roof_bottom.roof_quarter'], hGrade, 'Roof', ROOF_PALETTE)
      || '<div style="font-size:10px; color:#94a3b8; font-style:italic; padding:4px 0;">-</div>';
    ...
    if (is1x1SideOption) {
      ...
      sliceKeys.forEach(sk => {
        ...
        wallStackHtml += `
          <div style="width:100%; border-top:1px dashed #cbd5e1; padding-top:3px; margin-top:3px;">
      ...
      courses.forEach(course => {
        ...
        wallStackHtml += `
          <div style="width:100%; border-top:1px dashed #cbd5e1; padding-top:3px; margin-top:3px;">
            <div style="font-size:8.5px; font-weight:800; color:#0369a1; background:#e0f2fe; border-radius:3px; padding:1px 4px; display:inline-block; margin-bottom:2px;">${course}</div>
            <div style="display:flex; gap:2px; align-items:flex-start;">
  ...
  let html = `
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; padding: 6px 10px; gap: 8px; flex-wrap: wrap;">
      <div style="display: flex; align-items: center; gap: 6px;">
        ...
        <div style="display: flex; gap: 3px; flex-wrap: wrap;">
          <button type="button" onclick="setMatrixHeightFilter('ALL')" style="padding: 3px 8px; font-size: 10.5px; font-weight: 700; border-radius: 4px; border: 1px solid ${currentFilter === 'ALL' ? '#0284c7' : '#cbd5e1'}; background: ${currentFilter === 'ALL' ? '#0284c7' : '#ffffff'}; color: ${currentFilter === 'ALL' ? '#ffffff' : '#475569'}; cursor: pointer;">All (1mH~5mH)</button>
          <button type="button" onclick="setMatrixHeightFilter('LOW')" style="padding: 3px 8px; font-size: 10.5px; font-weight: 700; border-radius: 4px; border: 1px solid ${currentFilter === 'LOW' ? '#0284c7' : '#cbd5e1'}; background: ${currentFilter === 'LOW' ? '#0284c7' : '#ffffff'}; color: ${currentFilter === 'LOW' ? '#ffffff' : '#475569'}; cursor: pointer;">Low (1~2mH)</button>
          <button type="button" onclick="setMatrixHeightFilter('MID')" style="padding: 3px 8px; font-size: 10.5px; font-weight: 700; border-radius: 4px; border: 1px solid ${currentFilter === 'MID' ? '#0284c7' : '#cbd5e1'}; background: ${currentFilter === 'MID' ? '#0284c7' : '#ffffff'}; color: ${currentFilter === 'MID' ? '#ffffff' : '#475569'}; cursor: pointer;">Mid (2.5~3.5mH)</button>
          <button type="button" onclick="setMatrixHeightFilter('HIGH')" style="padding: 3px 8px; font-size: 10.5px; font-weight: 700; border-radius: 4px; border: 1px solid ${currentFilter === 'HIGH' ? '#0284c7' : '#cbd5e1'}; background: ${currentFilter === 'HIGH' ? '#0284c7' : '#ffffff'}; color: ${currentFilter === 'HIGH' ? '#ffffff' : '#475569'}; cursor: pointer;">High (4~5mH)</button>
        </div>
      </div>
    ...
    <div style="width: 100%; overflow-x: auto; border: 1px solid #cbd5e1; border-radius: 6px; background: #ffffff;">
      <table style="width: 100%; min-width: 0; border-collapse: collapse; table-layout: fixed; font-size: 11px;">
        <thead>
          <tr style="background: #f1f5f9; border-bottom: 2px solid #cbd5e1;">
            <th style="width: 75px; padding: 6px 2px; font-weight: 800; font-size: 10.5px; color: #1e293b; text-align: center; border-right: 2px solid #cbd5e1; background: #e2e8f0;">Height</th>
            ${visibleGrades.map(hGrade => {
              const isOdd = hGrade.includes('.5');
              return `<th style="padding: 6px 2px; font-weight: 800; font-size: 11px; color: #0f172a; text-align: center; background: ${isOdd ? '#dbeafe' : '#f1f5f9'}; border-right: 1px solid #cbd5e1;">${hGrade}</th>`;
            }).join('')}
          </tr>
        </thead>
        <tbody>
          ${isBasicOption ? `
            <!-- Roof Row -->
            <tr style="border-bottom: 1px solid #cbd5e1;">
              <td style="font-weight: 800; font-size: 10.5px; color: #15803d; text-align: center; background: #f0fdf4; border-right: 2px solid #cbd5e1; vertical-align: middle; padding: 6px 2px;">Roof</td>
              ${visibleGrades.map(hGrade => {
                const isOdd = hGrade.includes('.5');
                return `<td style="padding: 4px 2px; text-align: center; vertical-align: middle; background: ${isOdd ? '#f0f9ff' : '#ffffff'}; border-right: 1px solid #cbd5e1;">${roofHtmlMap[hGrade]}</td>`;
              }).join('')}
            </tr>

            <!-- Manhole Row -->
            <tr style="border-bottom: 1px solid #cbd5e1;">
              <td style="font-weight: 800; font-size: 10.5px; color: #b45309; text-align: center; background: #fffbeb; border-right: 2px solid #cbd5e1; vertical-align: middle; padding: 6px 2px;">Manhole</td>
              ${visibleGrades.map(hGrade => {
                const isOdd = hGrade.includes('.5');
                return `<td style="padding: 4px 2px; text-align: center; vertical-align: middle; background: ${isOdd ? '#f0f9ff' : '#ffffff'}; border-right: 1px solid #cbd5e1;">${manholeHtmlMap[hGrade]}</td>`;
              }).join('')}
            </tr>

            <!-- Bottom Row -->
            <tr style="border-bottom: 1px solid #cbd5e1;">
              <td style="font-weight: 800; font-size: 10.5px; color: #334155; text-align: center; background: #f8fafc; border-right: 2px solid #cbd5e1; vertical-align: middle; padding: 6px 2px;">Bottom</td>
              ${visibleGrades.map(hGrade => {
                const isOdd = hGrade.includes('.5');
                return `<td style="padding: 4px 2px; text-align: center; vertical-align: middle; background: ${isOdd ? '#f0f9ff' : '#ffffff'}; border-right: 1px solid #cbd5e1;">${bottomHtmlMap[hGrade]}</td>`;
              }).join('')}
            </tr>

            <!-- Drain Row -->
            <tr>
              <td style="font-weight: 800; font-size: 10.5px; color: #334155; text-align: center; background: #f8fafc; border-right: 2px solid #cbd5e1; vertical-align: middle; padding: 6px 2px;">Drain</td>
              ${visibleGrades.map(hGrade => {
                const isOdd = hGrade.includes('.5');
                return `<td style="padding: 4px 2px; text-align: center; vertical-align: middle; background: ${isOdd ? '#f0f9ff' : '#ffffff'}; border-right: 1px solid #cbd5e1;">${drainHtmlMap[hGrade]}</td>`;
              }).join('')}
            </tr>
          ` : isPartitionOption ? `
            <tr>
              <td style="font-weight: 800; font-size: 10.5px; color: #be185d; text-align: center; background: #fdf2f8; border-right: 2px solid #cbd5e1; vertical-align: middle; padding: 8px 2px;">
                Partition<br><span style="font-weight:600; font-size:8.5px; color:#9d174d;">(bottom→top)${sideMatrixOption === 3 ? '<br><br>Top Course<br>0.5/1M Alt' : ''}</span>
              </td>
              ${visibleGrades.map(hGrade => {
                const isOdd = hGrade.includes('.5');
                return `<td style="padding: 4px 2px; text-align: center; vertical-align: top; background: ${isOdd ? '#f0f9ff' : '#ffffff'}; border-right: 1px solid #cbd5e1;">${partitionHtmlMap[hGrade] || '<div style="font-size:9.5px; color:#94a3b8; font-style:italic; padding-top:10px;">No Partition</div>'}</td>`;
              }).join('')}
            </tr>
          ` : `
            <tr>
              <td style="font-weight: 800; font-size: 10.5px; color: #0369a1; text-align: center; background: #f0f9ff; border-right: 2px solid #cbd5e1; vertical-align: middle; padding: 8px 2px;">
                Wall<br><span style="font-weight:600; font-size:8.5px; color:#0284c7;">(bottom→top)</span>
              </td>
              ${visibleGrades.map(hGrade => {
                const isOdd = hGrade.includes('.5');
                return `<td style="padding: 4px 2px; text-align: center; vertical-align: top; background: ${isOdd ? '#f0f9ff' : '#ffffff'}; border-right: 1px solid #cbd5e1;">${wallStackHtmlMap[hGrade] || '<div style="font-size:9.5px; color:#94a3b8; font-style:italic; padding-top:10px;">No Wall</div>'}</td>`;
              }).join('')}
            </tr>
          `}
        </tbody>
