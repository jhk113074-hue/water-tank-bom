...
(function (global) {
  ...
  function render() {
    ...
    let html = `<table style="width:100%; border-collapse:collapse; font-size:12px;">
      <thead>
        <tr style="background:#f1f5f9; border-bottom:2px solid #334155;">
          <th style="padding:8px 10px; text-align:left;">Hole Drilling Spec</th>
    ...
    Object.keys(byOpeningCode).sort().forEach(code => {
      ...
      html += `<tr style="background:#fdf4ff; border-bottom:2px solid #d946ef;">
        <td colspan="3" style="padding:6px 10px; text-align:right; font-weight:800; font-size:11px; color:#a21caf;">Hole Drilling Spec [${escapeHtml(code)}] Subtotal</td>
  ...
  function printSheet() {
    ...
    win.document.write(`<html><head><title>Hole Drilling Spec Sheet</title></head><body>${container.outerHTML}</body></html>`);
    win.document.close();
    win.focus();
    win.print();
  }
